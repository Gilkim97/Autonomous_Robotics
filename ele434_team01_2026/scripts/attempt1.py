...
class AutonomousNavigation(Node):
    ...
    def odom_callback(self, msg_data:Odometry):
        ...
        if self.initialize == 0:
            ...
            self.yaw_offset = yaw
            print(f"initial_yaw:{self.yaw_offset}")
            self.initialize += 1
        (roll, pitch, yaw) = quaternion_to_euler(pose.orientation)
        self.current_x = pose.position.x - self.x_offset
        self.current_y = pose.position.y - self.y_offset
        self.current_yaw = yaw - self.yaw_offset
        print(f"Current x: {self.current_x},  Current y: {self.current_y}, Current yaw: {self.current_yaw}")
    ...
    def navigation_control(self):
        ...
        print(f"Current linear speed: {msg.twist.linear.x}, current angular speed: {msg.twist.angular.z}")
